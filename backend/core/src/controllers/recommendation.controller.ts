...
export const dismissRecommendationController = async (req: Request, resp: Response) => {
  try {
    if (!req.user?.id) {
      return resp.status(401).json({ 
        status: "error", 
    ...
    if (role !== "ADMIN" && role !== "BUILDING_MANAGER") {
        return resp.status(403).json({ 
          status: "error", 
          message: "You do not have permission to dismiss a reccomendation" 
        });
    }
    const { building_id, recommendation_id } = req.params;

    await dismissRecommendationService(req.user.id, building_id, recommendation_id);
