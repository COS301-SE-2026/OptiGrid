...
SEEDER_DAYS_BACK = 30
SEEDER_INTERVAL_SECONDS = 2.0
SEEDER_CHUNK_SECONDS = 14400.0
SEEDER_WRITE_BATCH_SIZE = 25000
    ...
    try:
        ...
        try:
            while current_start < end_ts:
                current_end = min(current_start + SEEDER_CHUNK_SECONDS, end_ts)
                ts_array = np.arange(current_start, current_end, SEEDER_INTERVAL_SECONDS)
                if len(ts_array) == 0:
                ...
                base_anomalies_expected = (num_points * SEEDER_INTERVAL_SECONDS * 2.0) / (30 * 24 * 3600)
                ...
                min_anomaly_points = max(1, int(round(60.0 / SEEDER_INTERVAL_SECONDS)))
                max_anomaly_points = max(min_anomaly_points + 1, int(round(300.0 / SEEDER_INTERVAL_SECONDS)))

                for _ in range(num_anomalies):
                    start_idx = rng.integers(0, num_points)
                    duration_pts = rng.integers(min_anomaly_points, max_anomaly_points + 1)
                ...
                for i in range(0, len(lines), SEEDER_WRITE_BATCH_SIZE):
                    chunk = "\n".join(lines[i:i + SEEDER_WRITE_BATCH_SIZE])
...
def _check_sensor_gap(s, query_api, days_back, end_time):
    ...
    query_days = max(days_back + 5, 1)
    q = f'from(bucket: "{INFLUXDB_BUCKET}") |> range(start: -{query_days}d) |> filter(fn: (r) => r._measurement == "energy_telemetry" and r.sensor_id == "{s["sensor_id"]}") |> last() |> keep(columns: ["_time"])'
    try:
        ...
        if last_ts:
            start_ts = last_ts.timestamp() + SEEDER_INTERVAL_SECONDS
        else:
            start_ts = (end_time - timedelta(days=days_back)).timestamp()
            
        if end_ts - start_ts > max(SEEDER_INTERVAL_SECONDS, 10.0):
            s["start_ts"] = start_ts
            s["end_ts"] = end_ts
            return s
    except Exception as e:
...
if __name__ == "__main__":
    ...
    if real_sensors:
        seed_calculated_buildings(sensors_data=real_sensors, days_back=SEEDER_DAYS_BACK)
    else:
