...
export async function POST(req: Request, { params }: { params: Promise<{ buildingId: string; recommendationId: string }> }) {
    ...
    const url = `${getCoreUrl()}/api/buildings/${encodeURIComponent(buildingId)}/recommendations/${encodeURIComponent(recommendationId)}/apply`;
    ...
    try {
        ...
        const payload = await resp.json().catch(() => ({
            message: resp.ok ? "Recommendation applied successfully" : "Unable to apply this recommendation",
        }));
