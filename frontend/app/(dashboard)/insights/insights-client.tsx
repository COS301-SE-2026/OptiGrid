...
// only the admins and building managers may approve or dismiss an insight
function canReview(role: string): boolean {
    return role === "ADMIN" || role === "BUILDING_MANAGER";
}

// a recommendation can only be actioned once while it is still pending and unexpired
function isActionable(recommendation: Recommendation): boolean {
    return recommendation.status === "Pending" && !isExpired(recommendation);
}
...
export default function InsightsClient({ role }: Readonly<{ role: string }>) {
    ...
    const { mutate: reviewRecommendation, variables: pendingReview, isPending: reviewPending } = useMutation({
        mutationFn: async ({ action, recommendationId }: { action: ReviewAction; recommendationId: string }) => {
            ...
            const payload = await response.json().catch(() => ({}));
            if (!response.ok) {
                throw new Error(payload.message || "Unable to complete this review.");
            }
            return payload;
        },
        onMutate: () => {
    ...
    return (
        <div>
            ...
            {reviewTarget && (
                <ReviewDialog
                    key={reviewTarget.recommendation_id}
