...
import { isTabSessionId, TAB_SESSION_HEADER } from "./tab-session";
import {
...
export async function forwardCredentials(request: Request, corePath: string, successMessage: string, failureMessage: string) {
	...
	try {
		const coreResponse = await fetch(`${CORE_URL}${corePath}`, {
			method: "POST",
			headers: {
		...
		const payload = (await coreResponse.json().catch(() => ({
			message: coreResponse.ok ? successMessage : failureMessage,
